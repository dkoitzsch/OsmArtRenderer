...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), watercolor_effect_(nullptr), current_canvas_(nullptr),
      canvas_(nullptr), canvas_pm_(nullptr), map_data_(nullptr), current_config_widget_(nullptr),
...
MainWindow::~MainWindow()
{
    ...
    for (auto it = watercolor_effect_configurations_.begin(); it != watercolor_effect_configurations_.end(); ++it) {
        delete it.value();
    }
...
void MainWindow::SetupUi()
{
    ...
    QObject::connect(btn_render_bw, &QPushButton::clicked, this, [this] {
        RenderMap(ChangeCanvasTo(canvas_));  // Think about passing the canvas as an argument.
    });
    QPushButton* btn_render_pm = new QPushButton(tr("Render P/M Map"));
    QObject::connect(btn_render_pm, &QPushButton::clicked, this, [this] {
        RenderMap(ChangeCanvasTo(canvas_pm_));
    });
    ...
    objects_config_tabs_->addTab(watercolor_effect_config_widget, "Watercolor Effect Configuration");
    QObject::connect(objects_config_tabs_, &QTabWidget::currentChanged, this, [this] {
       this->ObjectsListSelected();
    });
    ...
    QObject::connect(objects_list_, &QListWidget::itemSelectionChanged, this, &MainWindow::ObjectsListSelected);
    objects_list_->setFixedWidth(200);
    conf_layout->addWidget(objects_list_, 0, 0, /*rowSpan=*/1, /*columnSpan=*/1);
    conf_layout->addWidget(objects_config_tabs_, 0, 1, /*rowSpan=*/1, /*columnSpan=*/3);
...
{
    objects_repository_.AddObjects(osm::kBuildingsName, new osm::Objects(osm::kBuildingsName, {{.key="building", .value="*"}}));
    objects_repository_.AddObjects(osm::kDevelopedLandName, new osm::Objects(osm::kDevelopedLandName,
                                                                    {
                                                                        {.key="landuse", .value="*"},
    ...
    objects_repository_.AddObjects(osm::kHighwaysName, new osm::Objects(osm::kHighwaysName, {{.key="highway", .value="*"}}));
    objects_repository_.AddObjects(osm::kHighwaysExtName, new osm::Objects(osm::kHighwaysExtName,
                                                                    {
    ...
    objects_repository_.AddObjects(osm::kWaterwaysName, new osm::Objects(osm::kWaterwaysName,
                                                                    {
                                                                        {.key="waterway", .value="*"},
    ...
    objects_repository_.AddObjects(osm::kGreenlandName, new osm::Objects(osm::kGreenlandName,
                                                                    {
    ...
    objects_repository_.AddObjects(osm::kCoastlinesName, new osm::Objects(osm::kCoastlinesName, {{.key="natural", .value="coastline"}}));

    objects_repository_.ObjectsConfiguration(osm::kBuildingsName, new osm::ObjectsConfiguration(true, 1, Qt::black, true, Qt::black, true));    
    objects_repository_.ObjectsConfiguration(osm::kHighwaysName, new osm::ObjectsConfiguration(true, 4, Qt::black, true, Qt::black, true));
    objects_repository_.ObjectsConfiguration(osm::kHighwaysExtName, new osm::ObjectsConfiguration(true, 5, Qt::black, true, Qt::black, true));
    objects_repository_.ObjectsConfiguration(osm::kWaterwaysName, new osm::ObjectsConfiguration(true, 4, Qt::black, true, Qt::black, true));
    objects_repository_.ObjectsConfiguration(osm::kDevelopedLandName, new osm::ObjectsConfiguration(true, 1, QColor(230,230,230), true, QColor(230,230,230), true));
    objects_repository_.ObjectsConfiguration(osm::kGreenlandName, new osm::ObjectsConfiguration(true, 1, QColor(200,200,200), true, QColor(200,200,200), true));
    objects_repository_.ObjectsConfiguration(osm::kOceanName, new osm::ObjectsConfiguration(true, 1, Qt::black, true, Qt::black, true));
    objects_repository_.ObjectsConfiguration(osm::kLandmassName, new osm::ObjectsConfiguration(true, 1, QColor(250,250,250), true, QColor(250,250,250), true));

    /*QObject::connect(objects_repository_.ObjectsConfiguration(osm::kBuildingsName), &osm::ObjectsConfiguration::Updated, this, &MainWindow::ObjectsConfigUpdated);
    QObject::connect(objects_repository_.ObjectsConfiguration(osm::kHighwaysName), &osm::ObjectsConfiguration::Updated, this, &MainWindow::ObjectsConfigUpdated);
    QObject::connect(objects_repository_.ObjectsConfiguration(osm::kHighwaysExtName), &osm::ObjectsConfiguration::Updated, this, &MainWindow::ObjectsConfigUpdated);
    QObject::connect(objects_repository_.ObjectsConfiguration(osm::kWaterwaysName), &osm::ObjectsConfiguration::Updated, this, &MainWindow::ObjectsConfigUpdated);
    QObject::connect(objects_repository_.ObjectsConfiguration(osm::kOceanName), &osm::ObjectsConfiguration::Updated, this, &MainWindow::ObjectsConfigUpdated);
    QObject::connect(objects_repository_.ObjectsConfiguration(osm::kLandmassName), &osm::ObjectsConfiguration::Updated, this, &MainWindow::ObjectsConfigUpdated);*/
}
...
void MainWindow::SetupWatercolorEffect()
{
    ...
    // Note: Noisescale was 1 for all except 5 for developed land and greenland initially. But I think 0.2 is nicer.
    watercolor_effect_configurations_[osm::kBuildingsName] =
            new effects::WatercolorEffectConfiguration(osm::kBuildingsName,
                                                       noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                       QImage(":/textures/brown_42.jpg"));
    watercolor_effect_configurations_[osm::kHighwaysName]
            = new effects::WatercolorEffectConfiguration(osm::kHighwaysName,
                                                         noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                         QImage(":/textures/orange_11.jpg"));
    watercolor_effect_configurations_[osm::kHighwaysExtName]
            = new effects::WatercolorEffectConfiguration(osm::kHighwaysExtName,
                                                         noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                         QImage(":/textures/orange_10.jpg"));
    watercolor_effect_configurations_[osm::kWaterwaysName]
            = new effects::WatercolorEffectConfiguration(osm::kWaterwaysName,
                                                         noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                         QImage(":/textures/blue_24.jpg"));

    watercolor_effect_configurations_[osm::kDevelopedLandName]
            = new effects::WatercolorEffectConfiguration(osm::kDevelopedLandName,
                                                         noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                         QImage(":/textures/gray_04.jpg"));
    watercolor_effect_configurations_[osm::kGreenlandName]
            = new effects::WatercolorEffectConfiguration(osm::kGreenlandName,
                                                         noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                         QImage(":/textures/green_09.jpg"));

    watercolor_effect_configurations_[osm::kOceanName]
            = new effects::WatercolorEffectConfiguration(osm::kOceanName,
                                                         noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                         QImage(":/textures/blue_01.jpg"));
    watercolor_effect_configurations_[osm::kLandmassName]
            = new effects::WatercolorEffectConfiguration(osm::kLandmassName,
                                                         noise_scale_factor, final_blend_alpha, threshold_combiner_alpha, threshold_combiner_threshold,
                                                         QImage(":/textures/brown_40.jpg"));

    watercolor_effect_ = new effects::WatercolorEffect(watercolor_effect_configurations_);
}
...
void MainWindow::OpenFile()
{
    ...
    QString filename = QFileDialog::getOpenFileName(
                this, tr("Open OSM File"), homefolder,
                tr("OpenStreetMap Files (*.osm)"));
    if (!filename.isEmpty()) {
        ...
        //try {
        if (!objects_repository_.SetObjectsOrder({osm::kOceanName, osm::kLandmassName, osm::kDevelopedLandName,
...
void MainWindow::RenderMap(osm::Canvas* canvas)
{
    ...
    float a = osm::utils::GetAspectRatio(
                map_data_->MinLat(),
                map_data_->MaxLat(),
                map_data_->MinLon(),
                map_data_->MaxLon());
...
void MainWindow::RenderEffect(effects::Effect* effect)
{
    ...
    float a = osm::utils::GetAspectRatio(
                ...
                map_data_->MinLon(),
                map_data_->MaxLon());
    current_canvas_->setFixedSize(osm::Canvas::kMaxHeight * a, osm::Canvas::kMaxHeight);
...
void MainWindow::ObjectsListSelected()
{
    ...
    if (objects_config_tabs_->currentIndex() == 0) {
        if (objects_repository_.ObjectsConfiguration(item->text())) {
            objects_config_layout_->removeWidget(current_config_widget_);
            current_config_widget_ = objects_repository_.ObjectsConfiguration(item->text())->Widget();
        ...
        if (objects_repository_.ObjectsConfiguration(item->text())) {
            ...
            current_config_widget_ = watercolor_effect_configurations_[item->text()]->Widget();
            watercolor_effect_config_layout_->addWidget(current_config_widget_);
        } else {
            qDebug() << "No watercolor effect configuration found for" << item->text();
            return;
        }
    }
...
void MainWindow::GenerateCoastlines()
{
    ...
    osm::OceanLandmassFactory factory(
                coastlines, w, h,
                scale, {.x=diff_w, .y=diff_h},
                {.min_lat=map_data_->MinLat(), .max_lat=map_data_->MaxLat(),
                 .min_lon=map_data_->MinLon(), .max_lon=map_data_->MaxLon()});
    factory.Build();
    ...
    for (auto it = coastline_polygons.begin(); it != coastline_polygons.end(); ++it) {
    ...
    for (auto it = coastline_polygons.begin(); it != coastline_polygons.end(); ++it) {
        qDebug() << "\n-------------------\nDraw new polygon\n-------------------\n";
        for (auto it2 = it->begin()+1; it2 != it->end(); ++it2, ++idx) {
            //painter.setPen(p2);
            ...
            qDebug() << "Draw line from " << (it2-1)->x << "," << (it2-1)->y << "to" << (it2)->x << "," << (it2)->y;
        }
    }
    ...
    // Now generate nodes and ways and add everything to an OsmObjCollection instance.
    for (auto it = coastline_polygons.begin(); it != coastline_polygons.end(); ++it) {
        ...
        int node_idx = 0;
        for (auto it_nodes = polygon.begin() /*+4*/; it_nodes != polygon.end(); ++it_nodes, ++node_idx) {
    ...
    // Now "ways" contains all the newly created coastline polygons (which is the landmass).
    ...
    if (obj_landmass == nullptr) {
        obj_landmass = new osm::Objects(osm::kLandmassName, {}, osm::ObjectsTypes::LANDMASS);
